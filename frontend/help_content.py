...
def get_how_to_use_markdown() -> str:
    return """
miRAssist prioritizes candidate miRNA-target interactions using curated evidence, prediction models, CLIP/binding evidence, seed/site features, structure-aware features, TCGA context, and pathway filtering.
...
- Phenotype direction: `I overexpressed miR-21 in breast cancer cells and saw increased apoptosis. What target genes could explain this?`
...
- TCGA correlations are context evidence, not direct binding evidence.
...
- CLIP supports binding potential, not necessarily functional repression.
- TCGA anticorrelation supports context-consistent repression, not direct binding.

Novel mode:
- Novel mode excludes known miRTarBase functional interactions from the ranked list, though known interactions may still appear in context or caveats.
""".strip()
...
- `miRDB`: prediction-model support
- `TargetScan`: site-context prediction support
